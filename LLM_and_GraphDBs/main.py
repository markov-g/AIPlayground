...
# Cypher generation prompt -- chatgpt4 is not great at this - we probably need to fine-tune: https://towardsdatascience.com/fine-tuning-an-llm-model-with-h2o-llm-studio-to-generate-cypher-statements-3f34822ad5
...
def query_graph(user_input):
    try:
        # Attempt to initialize Neo4jGraph
        graph = Neo4jGraph(url=neo4j_url, username=neo4j_user, password=neo4j_password)
        logging.info("Neo4jGraph initialized successfully.")

    except Exception as e:
        logging.error("An error occurred while initializing Neo4jGraph: %s", e)
        raise e  # Re-raise the exception to halt execution if this step fails

    try:
        # Attempt to initialize GraphCypherQAChain
        ...
        logging.info("GraphCypherQAChain initialized successfully.")

    except Exception as e:
        logging.error("An error occurred while initializing GraphCypherQAChain: %s", e)
        raise e  # Re-raise the exception to halt execution if this step fails

    try:
        # Attempt to execute the chain with user input
        result = chain(user_input)
        logging.info("Chain executed successfully with result: %s", result)
        return result

    except Exception as e:
        logging.error("An error occurred while executing the query graph chain: %s", e)
        raise e
